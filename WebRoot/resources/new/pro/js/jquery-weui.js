...
+function ($) {
  ...
  var showTab = function(a) {
    ...
    var bd = $a.parents(".weui-tab1").find(".weui-tab__bd1");

    bd.find(".weui-tab__bd-item--active1").removeClass("weui-tab__bd-item--active1");

    $(href).addClass("weui-tab__bd-item--active1");
  }

  $.showTab = showTab;

  $(document).on("click", ".weui-navbar__item1, .weui-tabbar__item1", function(e) {
    var $a = $(e.currentTarget);
    var href = $a.attr("href");
    if($a.hasClass(ITEM_ON)) return;
    if(!/^#/.test(href)) return;
...
+ function($) {
  ...
  $(document).on("click", ".close-popup1, .weui-popup__overlay1", function() {
    $.closePopup();
  })
  .on("click", ".open-popup1", function() {
    $($(this).data("target")).popup();
  })
  .on("click", ".weui-popup__container1", function(e) {
    if($(e.target).hasClass("weui-popup__container1")) $.closePopup();
  })
