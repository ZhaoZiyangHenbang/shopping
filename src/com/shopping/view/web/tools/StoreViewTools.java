 ...
 public class StoreViewTools
 {
   ...
   public int generic_user_credit(String id) {
    /* int credit = 0;
     ...
     //分别 在注册方法和订单完成的方法里加上等级计算
     User user = this.userService.getObjById(CommUtil.null2Long(id));
     int credit=user.getUser_credit();
     /*for (int i = 0; i < ints.length - 1; i++) {
       int cc=user.getUser_credit();
       if ((ints[i].intValue() > user.getUser_credit()) || (ints[(i + 1)].intValue() < user.getUser_credit())) continue;
       credit = i + 1;
       break;
     }*/
