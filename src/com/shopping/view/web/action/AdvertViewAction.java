...
import org.springframework.web.servlet.ModelAndView;
 ...
 public class AdvertViewAction
 {
   ...
   public ModelAndView advert_invoke(HttpServletRequest request, HttpServletResponse response, String id,String store_id)
   {
       ...
       if (ap != null) {
         ...
         if (advs.size() > 0) {
           if (obj.getAp_type().equals("text")) {
             if (obj.getAp_show_type() == 0) {//zhi显示一条
           ...
           if (obj.getAp_type().equals("img")) {
             ...
             if (obj.getAp_show_type() == 0) {
               if(null!=store_id&&!"".equals(store_id)){
                 ...
                 Long ad_user_id=store.getUser().getId();
                 for (int i = 0; i <advs.size() ; i++) {
                 Long a=((Advert)advs.get(i)).getAd_user().getId();
                   if(((Advert)advs.get(i)).getAd_user().getId()==ad_user_id){
           ...
           if (obj.getAp_type().equals("indexslide")) {
               ...
               if (obj.getAp_show_type() == 1) {
                 if(null!=store_id&&!"".equals(store_id)) {
                   Store store = this.storeService.getObjById(CommUtil.null2Long(store_id));
                   Long ad_user_id = store.getUser().getId();
                   List<Advert> lis=null;
                   for (int i = 0; i <advs.size() ; i++) {
                   Long a=((Advert)advs.get(i)).getAd_user().getId();
                     if(a==ad_user_id){
                       Advert ad=(Advert)advs.get(i);
                       obj.getAdvs().add(ad);
                     }
                   }
                  // int diandian=obj.getAdvs().size();
                 //  obj.setAdvs(lis);
                 }else {
                   Random random = new Random();
                   Set list = CommUtil.randomInt(advs.size(), 8);
