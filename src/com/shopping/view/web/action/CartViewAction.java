...
import com.self.common.base.JsonContent;
import com.shopping.core.annotation.SecurityMapping;
import com.shopping.core.domain.virtual.SysMap;
import com.shopping.core.mv.JModelAndView;
import com.shopping.core.query.support.IPageList;
import com.shopping.core.security.support.SecurityUserHolder;
import com.shopping.core.tools.*;
...
import com.shopping.pay.Global;
import com.shopping.pay.PayEntity;
...
public class CartViewAction {
	...
	private List<StoreCart> cart_calc(HttpServletRequest request) {//此方法就是获取storecate表里面的数据，也就是有几条数据，几个店铺
		...
		String cart_session_id = "";//定义一个sessionid 然后去查cooki 如果cooki的名字和session名字一致说明之前存过  就把cooki里面的值直接付给session，没啥用
		...
		if(user != null){
			...
			user_cart = this.storeCartService.query(
					"select obj from StoreCart obj where obj.deleteStatus =:deleteStatus and obj.user.id=:user_id and obj.sc_status=:sc_status", params, -1, -1);
		}
		//通过上面的查询  查出cookie_cart和user_cart或者user_cart或者cookie_cart,
	...
	@RequestMapping({ "/add_yunshu.htm" })
	public void addyunshuid(HttpServletRequest request, HttpServletResponse response,String yunshufangshi,String cartid,String valu) {

		if(yunshufangshi!="请选择配送方式"&&cartid!="cart"){

			StoreCart storeCart=this.storeCartService.getObjById(CommUtil.null2Long(cartid));
			storeCart.setYunshu_id(yunshufangshi);
			storeCart.setYunshufeiyong(BigDecimal.valueOf(CommUtil.null2Double(valu)));
			this.storeCartService.update(storeCart);
		}
	}
		//添加用户使用的优惠券到表里
		@RequestMapping({ "/add_couponid.htm" })
		public void addcouponid(HttpServletRequest request, HttpServletResponse response,String ci_id,String cart_id) {

				if(ci_id!=null&&!"".equals(ci_id)){
					if(ci_id=="0"||ci_id=="-1"){
						StoreCart storeCart = this.storeCartService.getObjById(CommUtil.null2Long(cart_id));
						storeCart.setCouponInfo(null);
						this.storeCartService.update(storeCart);
					}else {
						CouponInfo couponInfo = this.couponInfoService.getObjById(CommUtil.null2Long(ci_id));
						StoreCart storeCart = this.storeCartService.getObjById(CommUtil.null2Long(cart_id));
						storeCart.setCouponInfo(couponInfo);
						this.storeCartService.update(storeCart);
					}
				}
			}
	@RequestMapping("/payOrder.htm")
	public void payOrder(String payType, Double amount, String orderId, HttpServletResponse response){
		String out_trade_no = orderId;
		int  amounty = (int)Math.rint(amount*100);
		PayEntity pe = new PayEntity("订单支付",out_trade_no,amounty+"");
		if(payType.equals("wxmp")){
			pe.setReturn_url(Global.get("orderWX_return_url"));
		}
		if(payType.equals("ali")){
			pe.setReturn_url(Global.get("orderALI_return_url"));
		}
		...
		try {
			...
			writer.print(Json.toJson(map1, JsonFormat.compact()));
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	//是否为选中要结算的商品
	@RequestMapping({ "/cart_gc_status.htm" })
	public void cart_gc_status(HttpServletRequest request, HttpServletResponse response,String characters,Boolean isorno) {
			if(characters!=null){
				String[] ids=characters.split(",");
				for (String id:ids
					 ) {
					if (id!=null){
						GoodsCart goodscart=this.goodsCartService.getObjById(CommUtil.null2Long(id));
						if (null!=goodscart){
							if(isorno==true){
								goodscart.setGc_status(1);
							}else {
								goodscart.setGc_status(0);
							}
	...
	public void add_goods_cart(HttpServletRequest request, HttpServletResponse response, String id, String count, String price, String gsp, String buy_type) {
		...
		/*if(null!=id&&!"".equals(id)) {
			...
			params.put("of_id", null);
			List list = this.goodsCartService.query("select obj from GoodsCart obj where obj.goods.id=:id and obj.of_id=:of_id and obj.sc.user.id =:user_id", params, -1, -1);
			*//*if () {
		...
		goodsCart.setPrice(BigDecimal.valueOf(CommUtil.null2Double(price)));
		//对商品属性进行设置
		if(null!=gsp&&!"".equals(gsp)){
		String prop_cifi="";
		String[] gsp_ids = gsp.split(",");
		for (String gsp_id : gsp_ids) {
			GoodsSpecProperty gsp2=this.goodsSpecPropertyService.getObjById(CommUtil.null2Long(gsp_id));
			prop_cifi+=gsp2.getSpec().getName()+":"+gsp2.getValue()+"";
		}
		goodsCart.setSpec_info(prop_cifi);}
		Goods good=this.goodsService.getObjById(CommUtil.null2Long(id));
		goodsCart.setGoods(good);
		goodsCart.setGc_status(0);
		//对storeCart进行设置   根据用户id和商品的店铺id查询是否有数据，如果有就直接加入  没有就先生成再插入进去
		List<StoreCart> list =null;
		if(null!=SecurityUserHolder.getCurrentUser()) {
			Map map2 = new HashMap();
			Long m = SecurityUserHolder.getCurrentUser().getId();
			Long n = good.getGoods_store().getId();
			map2.put("user_id", SecurityUserHolder.getCurrentUser().getId());
			map2.put("store_id", good.getGoods_store().getId());
			map2.put("deleteStatus", CommUtil.null2Boolean(false));
			 list = this.storeCartService.query("select obj from StoreCart obj where obj.deleteStatus =:deleteStatus and obj.user.id=:user_id and obj.store.id=:store_id", map2, -1, -1);
		}
		if(list.size()>0){
			for (StoreCart sc:list) {
				//更新storecart
				BigDecimal a=sc.getTotal_price();
				BigDecimal b1 = new BigDecimal(price);
				BigDecimal b2 = new BigDecimal(count);
				Double b=b1.multiply(b2).doubleValue();
				BigDecimal b3 = new BigDecimal(a.toString());
				BigDecimal b4 = new BigDecimal(b.toString());
				BigDecimal c= BigDecimal.valueOf(b1.add(b2).doubleValue());

				sc.setTotal_price(c);
				this.storeCartService.update(sc);
				//把storecart的id写入goodscart表中
				goodsCart.setSc(sc);
			}
			...
			BigDecimal b2 = new BigDecimal(count);
			Double b=b1.multiply(b2).doubleValue();
			sc.setTotal_price(BigDecimal.valueOf(b));

			sc.setStore(good.getGoods_store());
		...
		Map params = new HashMap();
		params.put("user_id", SecurityUserHolder.getCurrentUser().getId());
		params.put("goods_id", good.getId());
		params.put("deleteStatus", CommUtil.null2Boolean(false));
		List<GoodsCart> goodsCartList= new ArrayList<>();
		if(goodsCart.getSpec_info()!=null){
			params.put("spec_info", goodsCart.getSpec_info());
			goodsCartList = this.goodsCartService.query("select obj from GoodsCart obj where obj.deleteStatus=:deleteStatus and obj.sc.user.id= :user_id and obj.goods.id= :goods_id and obj.of_type=0 and obj.spec_info =:spec_info",params,-1,-1);
		}else{
			goodsCartList = this.goodsCartService.query("select obj from GoodsCart obj where obj.deleteStatus=:deleteStatus and obj.sc.user.id= :user_id and obj.goods.id= :goods_id and obj.of_type=0",params,-1,-1);
		}
		int count1=0;
		if (goodsCartList.size()>0) {
			count1+=goodsCartList.get(0).getCount();
			goodsCart.setId(goodsCartList.get(0).getId());
			//this.goodsCartService.delete(CommUtil.null2Long(goodsCartList.get(i).getId()));
		}
		goodsCart.setCount(CommUtil.null2Int(count)+count1);
		goodsCart.setOf_type("0");
		this.goodsCartService.update(goodsCart);
...
//	 * @param response
//	 */
//	@RequestMapping({ "/daifahuo.htm" })
//	public ModelAndView daifahuo(HttpServletRequest request, HttpServletResponse response,String id,String goodsid,Long idd) {
//		ModelAndView mv = new JModelAndView("newwap/Myorder_backorders.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);
//		Address ad = addressService.getObjById(CommUtil.null2Long(idd));
//		mv.addObject("ad",ad);
//		Goods gc = goodsService.getObjById(CommUtil.null2Long(goodsid));
//		mv.addObject("gc",gc);
...
//	}
	...
	public void goods_count_adjust(HttpServletRequest request, HttpServletResponse response, String cart_id, String store_id, String count) {
			...
			if (goods.getGroup_buy() == 2) {
				...
				if (gg.getGg_count() >= CommUtil.null2Int(count))
					for (StoreCart sc1 : cart) { // sc = (StoreCart)gc.next();
						for (int i = 0; i < sc1.getGcs().size(); i++) {
							GoodsCart art = sc1.getGcs().get(i);
							GoodsCart gc1 = art;
								if (art.getId().toString().equals(cart_id)) {
								sc1.setTotal_price(
										BigDecimal.valueOf(CommUtil.add(sc1.getTotal_price(),
				...
				for (StoreCart scart : cart) {
					for (int i = 0; i < scart.getGcs().size(); i++) {
						GoodsCart gcart = scart.getGcs().get(i);
						GoodsCart gc1 = gcart;
						if (gcart.getId().toString().equals(cart_id)) {
							scart.setTotal_price(BigDecimal.valueOf(CommUtil.add(scart.getTotal_price(), Double.valueOf((CommUtil.null2Int(count) - gcart.getCount())
						...
						if (gc.getId().toString().equals(cart_id)) {
							sscart.setTotal_price(BigDecimal.valueOf(CommUtil.add(sscart.getTotal_price(),
									Float.valueOf((CommUtil.null2Int(count) - gc.getCount())
											* CommUtil.null2Float(gc.getGoods().getCombin_price())))));
		...
		((Map) map).put("goods_inventory", goods.getGoods_inventory());
	...
	public ModelAndView goods_cart1(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("user/second/26Goods_Cart.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
		...
		if( (shopping_view_type != null) && (!shopping_view_type.equals( "" )) && (shopping_view_type.equals( "wap" )) ) {
			mv = new JModelAndView("newwap/goods_cartZ/goods_cart_manageZ.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);
			/*mv = new JModelAndView("wap/goods_cart1.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);*/
		}
		//List<StoreCart> cart = cart_calc(request);
		//List<StoreCart> user_cart = new ArrayList<StoreCart>();
		List<StoreCart> cart = new ArrayList<StoreCart>();
		...
		if(user!=null) {
			...
			cart = this.storeCartService.query("select obj from StoreCart obj where obj.deleteStatus =:deleteStatus and obj.user.id=:user_id and obj.sc_status=:sc_status", params, -1, -1);
		}

		if(cart != null) {
			int num=cart.size();
			for (int j = 0; j < num; j++) {
				StoreCart scZ = cart.get(j);
				int num2=scZ.getGcs().size();
				for (int i = 0; i < num2; i++) {
					if(scZ.getGcs().get(i)!=null) {
						if (scZ.getGcs().get(i).getGc_status() == 2) {
							scZ.getGcs().remove(i);
							num2--;
							i--;
						}
		...
		if (cart.size() >0) {
			Store store = SecurityUserHolder.getCurrentUser().getStore() != null ? SecurityUserHolder.getCurrentUser().getStore() : null;
			if (store != null) {//删除自己店铺里面的商品
				for (StoreCart sc : cart) {
					if (sc.getStore().getId().equals(store.getId())) {//如果购物车里存的店铺id和登陆用户的店铺id一样
			...
			if( (shopping_view_type != null) && (!shopping_view_type.equals( "" )) && (shopping_view_type.equals( "wap" )) ) {
				mv = new JModelAndView("newwap/goods_cartZ/goods_cart_status.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);
			}
	...
	public ModelAndView goods_cart2(HttpServletRequest request, HttpServletResponse response ,String ids,String communt,String che) {
		ModelAndView mv = new JModelAndView("user/second/27Goods_Cart2.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
		//ModelAndView mv = new JModelAndView("goods_cart2.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);
		String shopping_view_type = CommUtil.null2String( request.getSession().getAttribute( "shopping_view_type" ) );
		if( (shopping_view_type != null) && (!shopping_view_type.equals( "" )) && (shopping_view_type.equals( "wap" )) ) {
			mv = new JModelAndView("wap/goods_cart2.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);
		//	mv = new JModelAndView("newwap/goods_cartZ/goods_cart_isOrder.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);
		}
		mv.addObject("ids",ids);
		mv.addObject("communt",communt);
		mv.addObject("che",che);
		List<StoreCart> cart2 = new ArrayList<StoreCart>();
		StoreCart storeCart2=new StoreCart();
		...
		if(null!=ids&&!"".equals(ids)){
			for (String id : ids.split(",")) {
				if (!CommUtil.null2String(id).equals("")) {
					GoodsCart goodscart = this.goodsCartService.getObjById(CommUtil.null2Long(id));
					store=goodscart.getSc().getStore();
					list.add(goodscart);
				}
			}
			if(null!=list&&list.size()>0) {
				storeCart2.setGcs(list);
				cart2.add(storeCart2);
			}
		}

		List<StoreCart> sc=new ArrayList<StoreCart>();
		sc=cart2;
		if (sc != null) {
			Map params = new HashMap();
			params.put("user_id", SecurityUserHolder.getCurrentUser().getId());
			params.put("type",CommUtil.null2Int(0));
			params.put("deleteStatus",CommUtil.null2Boolean(false));
			List addrs = this.addressService.query("select obj from Address obj where obj.deleteStatus =:deleteStatus and obj.user.id=:user_id and obj.type=:type order by obj.addTime desc", params, -1, -1);
			mv.addObject("addrs", addrs);
			//用户积分
			int integ=SecurityUserHolder.getCurrentUser().getIntegral();
			mv.addObject("integ",integ);
			int dui=SecurityUserHolder.getCurrentUser().getIntegral_duihuan();
			if(dui==0) dui=100;
			int inte=integ;
			Double qian=Double.valueOf(inte)/Double.valueOf(dui);
			mv.addObject("qian",qian);
			...
			params.put("user_id", SecurityUserHolder.getCurrentUser().getId());
			params.put("deleteStatus", CommUtil.null2Boolean(false));
			List<CouponInfo> list2=this.couponInfoService.query("select obj from CouponInfo obj where obj.goods.id is not null and obj.deleteStatus =:deleteStatus and obj.coupon.coupon_begin_time<=:coupon_begin_time and obj.coupon.coupon_end_time>=:coupon_end_time and obj.user.id=:user_id",params,-1,-1);
			...
			for (StoreCart scc:sc) {
				List<GoodsCart> goodCarts = scc.getGcs();
			...
			//商家购物券
			List<CouponInfo> li = new ArrayList<>();
			Map map=new HashMap();
			map.put("deleteStatus", CommUtil.null2Boolean(false));
			map.put("status", CommUtil.null2Int("0"));
			map.put("user_id", SecurityUserHolder.getCurrentUser().getId());
			map.put("coupon_begin_time", new Date());
			map.put("coupon_end_time", new Date());
			List<CouponInfo> couponInfoList = couponInfoService.query("select obj from CouponInfo obj where obj.deleteStatus =:deleteStatus and obj.goods.id is null and obj.status=:status and obj.user.id=:user_id and obj.coupon.coupon_begin_time<=:coupon_begin_time and obj.coupon.coupon_end_time>=:coupon_end_time",map,-1,-1);
			if(couponInfoList.size()>0) {
				for (CouponInfo couponInfo : couponInfoList) {
					BigDecimal a = couponInfo.getCoupon().getCoupon_order_amount();
					BigDecimal b = BigDecimal.valueOf(Double.valueOf(communt));
					if (a.compareTo(b) < 0) {
						li.add(couponInfo);
					}
				}
	...
	public ModelAndView goods_cart3(HttpServletRequest request, HttpServletResponse response,String coupon_id,String addr_id,String yunshufangshi,int jifenshu,String storeid,String idss,String yingfu,String che,String liuyan) throws Exception {
		ModelAndView mv = new JModelAndView("user/second/28Goods_Cart3.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
		...
		if( (shopping_view_type != null) && (!shopping_view_type.equals( "" )) && (shopping_view_type.equals( "wap" )) ) {
			...
			mv = new JModelAndView("newwap/goods_cartZ/goods_cart_pay.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);
		}
		if(null!=idss&&!"".equals(idss)){
			for (String id : idss.split(",")) {
				if (!CommUtil.null2String(id).equals("")) {
					GoodsCart goodscart = this.goodsCartService.getObjById(CommUtil.null2Long(id));
					int count=goodscart.getCount();
					Goods goo=this.goodsService.getObjById(goodscart.getGoods().getId());
					int count2=goo.getGoods_inventory();
					if(count>count2){
						mv = new JModelAndView("error.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);

						mv.addObject("op_title", goo.getGoods_name()+"商品库存不足，请更改购买数量后重新提交");
		...
		of.setOrder_id(SecurityUserHolder.getCurrentUser().getId() + CommUtil.formatTime("yyyyMMddHHmmss", new Date()));
		...
		of.setStore(store);
		CouponInfo cou=this.couponInfoService.getObjById(CommUtil.null2Long(coupon_id));
		of.setCi(cou);
		//设置店铺优惠券为已经使用
		if(null!=cou) cou.setStatus(1);
		this.couponInfoService.update(cou);
		of.setTransport(yunshufangshi);
		of.setMsg(liuyan);
		//是否用积分抵现
		Double inteMoney=0.00;
		String[] values=request.getParameterValues("jifen");
		if(values!=null){
				for (String value:values) {
					inteMoney+=Double.parseDouble(value);
				}

		}
		of.setInte_money(BigDecimal.valueOf(inteMoney));
		String ids="";
		Double money=0.00;
		List<GoodsCart> goodsCartList=new ArrayList<>();
		if(null!=idss&&!"".equals(idss)){
			for (String id : idss.split(",")) {
				if (!CommUtil.null2String(id).equals("")) {
					...
					Long goods_id=goodscart.getGoods().getId();
					Map params=new HashMap();
					params.put("coupon_begin_time", new Date());
					params.put("coupon_end_time", new Date());
					params.put("user_id", SecurityUserHolder.getCurrentUser().getId());
					...
					params.put("deleteStatus", CommUtil.null2Boolean(false));
					List<CouponInfo> list_ci=this.couponInfoService.query("select obj from CouponInfo obj where obj.deleteStatus =:deleteStatus and obj.goods.id =:goods_id and obj.coupon.coupon_begin_time<=:coupon_begin_time and obj.coupon.coupon_end_time>=:coupon_end_time and obj.user.id=:user_id",params,-1,-1);
					if(list_ci.size()>0){
						for (CouponInfo coup:list_ci) {
							//商品优惠券集合
							ids +=coup.getId().toString()+',';
							//商品优惠总额
							money+=Double.parseDouble(coup.getCoupon().getCoupon_amount().toString());
							//设置优惠券为已经使用
							coup.setStatus(1);
							this.couponInfoService.update(coup);
						}
					}
				}
			}
			of.setGcs(goodsCartList);
			//of.setGoods_amount(BigDecimal.valueOf(t));
			of.setGoods_coup_ids(ids);

		}
		if(null!=cou) {
			if (null == cou.getCoupon().getCoupon_amount() && "".equals(cou.getCoupon().getCoupon_amount())) {
				of.setGoods_coup_money(BigDecimal.valueOf(money));
			} else {
				double amount = cou.getCoupon().getCoupon_amount().intValue();
				of.setGoods_coup_money(BigDecimal.valueOf(money + amount));
			}
		}
		//应付金额设置
		of.setTotalPrice(BigDecimal.valueOf(CommUtil.null2Double(yingfu)));
		of.setOrder_type("web");
		BigDecimal countmoney = BigDecimal.valueOf(money).add(BigDecimal.valueOf(inteMoney)).add(of.getTotalPrice());
		//inteMoney+money totalPrice
		of.setGoods_amount(countmoney);
		this.orderFormService.save(of);//订单保存
		...
		for (String id : idss.split(",")) {
			if (!CommUtil.null2String(id).equals("")) {
				GoodsCart goodscart = this.goodsCartService.getObjById(CommUtil.null2Long(id));
				//String goodsname=goodscart.getGoods().getGoods_name();
				//list.add(goodsname);
				goodscart.setGc_status(2);
				goodscart.setOf(of);
				goodscart.setOf_type("1");
				goodscart.setEvaluate_status("0");
				this.goodsCartService.update(goodscart);
				//减库存
				Goods good=this.goodsService.getObjById(goodscart.getGoods().getId());
				int a=good.getGoods_inventory()-goodscart.getCount();
				good.setGoods_inventory(a);
		...
		if(values!=null) {
			...
			user.setIntegral(user.getIntegral() - jifenshu);
			this.userService.update(user);
		}
		OrderFormLog ofl = new OrderFormLog();//添加订单信息表
	...
	public ModelAndView order_pay_view(HttpServletRequest request, HttpServletResponse response, String id,String goodsid,Long idd) {
		...
		String shopping_view_type = CommUtil.null2String( request.getSession().getAttribute( "shopping_view_type" ) );
		if( (shopping_view_type != null) && (!shopping_view_type.equals( "" )) && (shopping_view_type.equals( "wap" )) ) {
			//mv = new JModelAndView("newwap/wodedingdan_daifukuan.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);
			mv = new JModelAndView("newwap/goods_cartZ/goods_cart_pay.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);
		}
		Address ad = addressService.getObjById(CommUtil.null2Long(idd));
		mv.addObject("ad",ad);
...
//支付宝支付的表单提交方法
	...
	public ModelAndView order_pay(HttpServletRequest request, HttpServletResponse response, String payType, String order_id) {
		...
		if (of.getOrder_status() == 10) {
			if (CommUtil.null2String(payType).equals("")) {
				...
				int Money=CommUtil.null2Int( of.getTotalPrice());
				int fen=Money/l;
				of.setJifen(CommUtil.null2String(fen));
	...
	public String paymentSubmit(HttpServletRequest request, HttpServletResponse response, String payType, String order_id) {
    ...
    public void wxcodepay(HttpServletRequest request, HttpServletResponse response, String order_id){
		...
		if (of.getOrder_status() == 10) {
			...
			SortedMap<Object, Object> parameters = new TreeMap<Object, Object>();
			parameters.put("appid",payment.getWeixin_appId());// 公众账号id//payment.getWeixin_appId()
			parameters.put("mch_id",payment.getWeixin_partnerId() );// 商户号//payment.getWeixin_partnerId()
			...
			if (ret) {
				...
				for (GoodsCart gc : of.getGcs()) {
					...
					if (goods.getInventory_type().equals("all")) {
						...
						List list = Json.fromJson(ArrayList.class, goods.getGoods_inventory_detail());
	...
	public ModelAndView order_finish(HttpServletRequest request, HttpServletResponse response, String order_id) {
		...
		if(obj.getOrder_status()==15) {

			User user = SecurityUserHolder.getCurrentUser();
			CombinLog cl = new CombinLog();
			cl.setAddTime(new Date());
			cl.setStatus(CommUtil.null2String(0));
			cl.setMoney(obj.getTotalPrice());
			cl.setUser(user);
			cl.setType(CommUtil.null2String(1));
			cl.setInfo("订单付款！");

			this.combinLogService.save(cl);
		}
	...
	public ModelAndView cart_address(HttpServletRequest request, HttpServletResponse response, String id, String store_id) {
		...
		String shopping_view_type = CommUtil.null2String( request.getSession().getAttribute( "shopping_view_type" ) );
		if( (shopping_view_type != null) && (!shopping_view_type.equals( "" )) && (shopping_view_type.equals( "wap" )) ) {
			mv = new JModelAndView("newwap/63tianjiadizhi.html", this.configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 1, request, response);
		}
	...
	public void order_address(HttpServletRequest request, HttpServletResponse response, String addr_id,
	   ...
	   public ModelAndView address_edit(HttpServletRequest request, HttpServletResponse response, String id, String currentPage, String store_id) {
		 ...
		 ModelAndView mv = new JModelAndView("address_edit.html", this.configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 1, request, response);
		 String shopping_view_type = CommUtil.null2String( request.getSession().getAttribute( "shopping_view_type" ) );
		 if( (shopping_view_type != null) && (!shopping_view_type.equals( "" )) && (shopping_view_type.equals( "wap" )) ) {
			 mv = new JModelAndView("newwap/64bianjidizhi.html", this.configService.getSysConfig(),
						this.userConfigService.getUserConfig(), 1, request, response);
		 }
	     List areas = this.areaService.query("select obj from Area obj where obj.parent.id is null", null, -1, -1);
	...
	@SecurityMapping(display = false, rsequence = 0, title = "地址切换", value = "/order_address.htm*", rtype = "buyer", rname = "购物流程3", rcode = "goods_cart", rgroup = "在线购物")
	@RequestMapping({ "/goods_cart2_inventory.htm" })
	public void goods_cart2_inventory( HttpServletResponse response, String count) {
		Map sms= new HashMap();
		boolean success = true;
		//co（id,,数量）
		String[] co = count.split("!");
		StringBuffer sb = new StringBuffer();
		for (String st:co) {
			String[] c = st.split(",,");
			Map ztc_map = new HashMap();
			ztc_map.put("goods_id", CommUtil.null2Long(c[0]));
			List<Goods> ztc_goods = this.goodsService.query("select obj from Goods obj where obj.id=:goods_id", ztc_map, -1, -1);
			Goods goods = ztc_goods.get(0);
			System.out.println("-----------------"+goods.getGoods_inventory());
			if(Integer.valueOf(c[1])>ztc_goods.get(0).getGoods_inventory()){
				sb.append("商品名为【"+goods.getGoods_name()+"】库存不足，当前库存为【"+goods.getGoods_inventory()+"】,请及时修改，及时购买！！！");
				success = false;
			}
		}
		sms.put("successs",success);
		sms.put("msgs",sb.toString());
		response.setContentType("text/plain");
		response.setHeader("Cache-Control", "no-cache");
		response.setCharacterEncoding("UTF-8");
		try {
			PrintWriter writer = response.getWriter();
			writer.print(Json.toJson(sb.toString(), JsonFormat.compact()));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
