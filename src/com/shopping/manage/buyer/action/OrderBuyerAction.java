...
public class OrderBuyerAction {
    ...
    public ModelAndView order(HttpServletRequest request, HttpServletResponse response, String currentPage, String order_id, String beginTime, String endTime, String order_status,String goods_name,String userName) {
        ModelAndView mv = new JModelAndView("user/second/42Buyer_Order.html", this.configService.getSysConfig(),
        ...
        if ((shopping_view_type != null) && (!shopping_view_type.equals("")) && (shopping_view_type.equals("wap"))) {
            mv = new JModelAndView("newwap/wodedingdan.html",
        ...
        if (!CommUtil.null2String(goods_name).equals("")) {
            ...
            List<GoodsCart> list=orderf.getGcs();
            for (GoodsCart gc:list) {
                ofqo.addQuery("gc.goods_name", new SysMap("goods_name", "%" + goods_name + "%"), "like");
            }
            mv.addObject("goods_name", goods_name);
        }
        if (!CommUtil.null2String(userName).equals("")) {
            ofqo.addQuery("obj.addr.trueName", new SysMap("userName", "%" +userName + "%"), "like");
        ...
        CommUtil.saveIPageList2ModelAndView(url+"/buyer/order.htm", "", "", pList, mv);
        return mv;
    }
    ...
    public ModelAndView order_cancel(HttpServletRequest request, HttpServletResponse response, String id, String currentPage) {
        ...
        if ((shopping_view_type != null) && (!shopping_view_type.equals("")) && (shopping_view_type.equals("wap"))) {
            mv = new JModelAndView("newwap/wodedingdan_yiquxiao.html",
                    this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);
        }
    ...
    public String order_cofirm_save(HttpServletRequest request, HttpServletResponse response, String id, String currentPage) throws Exception {
        ...
        if (obj.getUser().getId().equals(SecurityUserHolder.getCurrentUser().getId())) {
            ...
            int fen1=0;
            if (null!=obj.getJifen()&&!"".equals(obj.getJifen())) {
                fen1=Integer.parseInt(obj.getJifen());
            }
            int fen2=(int)((obj.getInte_money().doubleValue())*10);
            user.setIntegral(user.getIntegral()+fen1-fen2);
            if (null!=user.getPay_money()) {
                user.setPay_money(user.getPay_money().add(obj.getTotalPrice()));
            }else {
                user.setPay_money(obj.getTotalPrice());
            }
            this.userService.update(user);
            CouponInfo couponInfo=obj.getCi();
            if (null!=couponInfo) {
               couponInfo.setOrderForm_id(obj);
                couponInfo.setStatus(1);
                this.couponInfoService.update(couponInfo);
            }
            BigDecimal one = obj.getUser().getPay_money();
            obj.getUser().setPay_money(one);
            //当前用户的下线数
            Integer num = 0;
            Map params = new HashMap();
            params.put("parent", obj.getUser());
            //二级的金额
            List<User> twoUserList = this.userService.query("select obj from User obj where obj.parent=:parent", params, -1, -1);
            for (User use:twoUserList) {
                if(use.getUser_credit()!=0){
                    num++;
                }
                BigDecimal two = use.getPay_money();
                one = one.add(two);
                params.clear();
                params.put("parent", use);
                //三级的金额
                List<User> threeUserList = this.userService.query("select obj from User obj where obj.parent=:parent", params, -1, -1);
                for (User us:threeUserList) {
                    if(us.getUser_credit()!=0){
                        num++;
                    }
                    BigDecimal three = us.getPay_money();
                    one = one.add(us.getPay_money());
                }
            }
            //判断
            SysConfig sysConfig = this.configService.getSysConfig();
            //用户的等级（按照金额划分）
            String creditMoney = fenXiaoUtil.getCredit("0",Math.round(one.doubleValue()),sysConfig.getUser_creditrule());
            //用户的等级（按照人数划分）
            String creditPerson = fenXiaoUtil.getCredit("1",num,sysConfig.getUser_creditrule());
            if(Integer.valueOf(creditMoney)>Integer.valueOf(creditPerson)){
                obj.getUser().setUser_credit(Integer.valueOf(creditMoney));
            }else{
                obj.getUser().setUser_credit(Integer.valueOf(creditPerson));
            }
            userService.update(obj.getUser());
        }
        String url = "redirect:order.htm?currentPage=" + currentPage;
        return url;
    }

    @SecurityMapping(display = false, rsequence = 0, title = "商品评价", value = "/buyer/goods_evaluate.htm*", rtype = "buyer", rname = "用户中心", rcode = "user_center", rgroup = "用户中心")
    @RequestMapping({"/buyer/goods_evaluate.htm"})
    public ModelAndView goods_evaluate(HttpServletRequest request, HttpServletResponse response,String currentPage, String orderBy, String orderType,String id) {
        ModelAndView mv = new JModelAndView(
                "user/default/usercenter/goods_evaluate.html", this.configService
                .getSysConfig(),
                this.userConfigService.getUserConfig(), 0, request, response);
        String shopping_view_type = CommUtil.null2String(request.getSession().getAttribute("shopping_view_type"));
        if ((shopping_view_type != null) && (!shopping_view_type.equals("")) && (shopping_view_type.equals("wap"))) {
        ...
        List of = this.orderFormService
                .query(
                        "select obj from OrderForm obj where user_id ='"+id+"' ",
                        null, -1, -1);
        mv.addObject("of", of);
        return mv;
    }
    ...
    public ModelAndView order_evaluate(HttpServletRequest request, HttpServletResponse response, String id) {
        ModelAndView mv = new JModelAndView(/*"user/default/usercenter/buyer_order_evaluate.html"*/
               "user/second/44buyer_order_evaluate.html", this.configService.getSysConfig(),
                this.userConfigService.getUserConfig(), 0, request, response);
        OrderForm obj = this.orderFormService.getObjById(CommUtil.null2Long(id));
    ...
    public ModelAndView order_evaluate_save(HttpServletRequest request, HttpServletResponse response, String id) throws Exception {
        ...
        if (obj.getUser().getId().equals(SecurityUserHolder.getCurrentUser().getId())) {
            /*if (obj.getOrder_status() == 40) {*/
                ...
                eva1.setAddTime(new Date());
                eva1.setService_evaluate(BigDecimal.valueOf(CommUtil.null2Double(request.getParameter("service_evaluate" + id))));
                eva1.setEvaluate_type("order");
                eva1.setEvaluate_user(SecurityUserHolder.getCurrentUser());
                eva1.setOf(obj);
                eva1.setEvaluate_order_info(request.getParameter("evaluate_order_info"));
                eva1.setPic_status("1");
                this.evaluateService.save(eva1);
                for (GoodsCart gc : obj.getGcs()) {
                    ...
                    Map map = new HashMap();
                    map = CommUtil.saveFileToServer(request, "brandLogo"+gc.getId(),
    ...
    public String order_delete(HttpServletRequest request, HttpServletResponse response, String id, String currentPage) throws Exception {
        ...
        if ((obj.getUser().getId().equals(SecurityUserHolder.getCurrentUser().getId())) && (obj.getOrder_status() == 0)) {
            for (GoodsCart gc : obj.getGcs()) {
                gc.getGsps().clear();
                gc.setDeleteStatus(CommUtil.null2Boolean(true));
                this.goodsCartService.update(gc);
            }
            obj.setDeleteStatus(CommUtil.null2Boolean(true));
            this.orderFormService.update(obj);
        }
        return "redirect:order.htm?currentPage=" + currentPage;
    }
    ...
    public ModelAndView order_view(HttpServletRequest request, HttpServletResponse response, String id) {
        ModelAndView mv = new JModelAndView("user/second/43Buyer_Order_View.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
        String shopping_view_type = CommUtil.null2String(request.getSession().getAttribute("shopping_view_type"));
        if ((shopping_view_type != null) && (!shopping_view_type.equals("")) && (shopping_view_type.equals("wap"))) {
            mv = new JModelAndView("newwap/wodedingdan_daifukuan2.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);
        }
        ...
        if (obj.getUser().getId().equals(SecurityUserHolder.getCurrentUser().getId())) {
            mv.addObject("obj", obj);
            //优惠
            BigDecimal a=obj.getGoods_amount();
            BigDecimal b=obj.getTotalPrice();
            double c=a.subtract(b).doubleValue();
            mv.addObject("c", c);
//jifen
            int l=this.configService.getSysConfig().getConsumptionRatio();
           int  m=obj.getGoods_amount().intValue();
    ...
    public ModelAndView order_ship_view(HttpServletRequest request, HttpServletResponse response, String id) {
        ModelAndView mv = new JModelAndView("user/default/usercenter/order_ship_view.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
        String shopping_view_type = CommUtil.null2String(request.getSession().getAttribute("shopping_view_type"));
        if ((shopping_view_type != null) && (!shopping_view_type.equals("")) && (shopping_view_type.equals("wap"))) {
            mv = new JModelAndView("newwap/wuliuxiangqing.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);
        }
...
//            }
    ...
    @RequestMapping({"/seller/order_refund_apply.htm"})
    public ModelAndView order_refund_apply(HttpServletRequest request, HttpServletResponse response, String id, String view) {
        ModelAndView mv = new JModelAndView("user/default/usercenter/buyer_rufund_apply.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
        OrderForm obj = this.orderFormService.getObjById(CommUtil.null2Long(id));
...
//卖家查看退款申请信息
    @RequestMapping({"/seller/order_refund_apply_kan.htm"})
    public ModelAndView order_refund_apply_kan(HttpServletRequest request, HttpServletResponse response, String id, String view) {
        ModelAndView mv = new JModelAndView("user/default/usercenter/buyer_rufund_apply.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
        OrderForm obj = this.orderFormService.getObjById(CommUtil.null2Long(id));
       // if (obj.getUser().getId().equals(SecurityUserHolder.getCurrentUser().getId())) {
            mv.addObject("obj", obj);
            List list=obj.getRls();
            if(list.size()>0) {
                RefundLog lis = (RefundLog) list.get(0);
                String a = lis.getRefund_log();
                mv.addObject("list", a);
            }
            if ((view != null) && (!view.equals("")))
                mv.addObject("view", Boolean.valueOf(true));
    ...
    @SecurityMapping(display = false, rsequence = 0, title = "买家退款申请保存", value = "/buyer/order_refund_apply_save.htm*", rtype = "buyer", rname = "用户中心", rcode = "user_center", rgroup = "用户中心")
    @RequestMapping({"/buyer/order_refund_apply_save.htm"})
    public String order_refund_apply_save(HttpServletRequest request, HttpServletResponse response, String id, String currentPage, String return_content) throws Exception {
        OrderForm obj = this.orderFormService.getObjById(CommUtil.null2Long(id));
