 ...
 public class GoodsClassManageAction
 {
   ...
   public ModelAndView goods_class_list(HttpServletRequest request, HttpServletResponse response, String currentPage,String bc_location, String orderBy, String orderType)
   {
     ...
     qo.addQuery("obj.bc_location", new SysMap("bc_location", CommUtil.null2String(bc_location)),"=");
     ...
     String map="&bc_location="+bc_location;
     CommUtil.saveIPageList2ModelAndView(url + "/admin/goods_class_list.htm", "", map, pList, mv);
   ...
   {
     ModelAndView mv = new JModelAndView("admin/blue/goods_class_add.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
     List gcs = this.goodsClassService.query("select obj from GoodsClass obj where obj.parent.id is null", null, -1, -1);
   ...
   public ModelAndView goods_class_edit(HttpServletRequest request, HttpServletResponse response, String id, String currentPage)
   {
     ...
     if ((id != null) && (!id.equals(""))) {
       ...
       List gcs = this.goodsClassService.query("select obj from GoodsClass obj where  obj.parent.id is null",
         null, -1, -1);
   ...
   public ModelAndView goods_class_save(HttpServletRequest request, HttpServletResponse response, String bc_location,String id, String pid, String goodsTypeId, String currentPage, String list_url, String add_url, String child_link)
   {
     WebForm wf = new WebForm();
     ...
     if (id.equals("")) {
       ...
       goodsClass.setBc_location(bc_location);
     } else {
   ...
   public ModelAndView goods_class_data(HttpServletRequest request, HttpServletResponse response, String pid, String currentPage) {
     ...
     List gcs = this.goodsClassService.query(
       "select obj from GoodsClass obj where obj.deleteStatus=:deleteStatus and obj.parent.id =:pid order by obj.sequence asc", map, -1, -1);
     mv.addObject("gcs", gcs);
     mv.addObject("currentPage", currentPage);
     return mv;
   }
   ...
   public String goods_class_del(HttpServletRequest request, String mulitId, String currentPage) {
     ...
     for (String id : ids) {
       if (!id.equals("")) {
         ...
         for (GoodsClass gc : gcs) {
           ...
           this.goodsClassService.update(gc);
           System.out.println("shanchuid"+gc.getId());
     ...
     return "redirect:goods_class_list.htm?bc_location=1";
   }
 
   @RequestMapping({"/admin/goods_class_verify.htm"})
   public void goods_class_verify(HttpServletRequest request, HttpServletResponse response, String className, String id, String bc_location)
   {
     boolean ret = true;
     Map params = new HashMap();
     ...
     List gcs = this.goodsClassService
       .query("select obj from GoodsClass obj where obj.className=:className and obj.id!=:id and obj.bc_location =:bc_location",
       params, -1, -1);
