...
import com.shopping.manage.admin.tools.MsgTools;
 ...
 public class GoodsManageAction
 {
   ...
   public ModelAndView goods_list(HttpServletRequest request, HttpServletResponse response, String currentPage,String bc_location, String orderBy,String goods_name, String orderType,String gc_id,String brand_id,Boolean store_recommend)
   {
     ...
     if(null!=brand_id&&!"".equals(brand_id)){
       qo.addQuery("obj.goods_brand.id", new SysMap("brand_id", CommUtil.null2Long(brand_id)), "=");
       params="&&brand_id="+CommUtil.null2Long(brand_id);
       mv.addObject("brand_id",brand_id);
     }
     if(null!=gc_id&&!"".equals(gc_id)){
       qo.addQuery("obj.gc.id", new SysMap("gc_id", CommUtil.null2Long(gc_id)), "=");
       params="&&brand_id="+CommUtil.null2Long(brand_id)+"&&gc_id="+CommUtil.null2Long(gc_id);
       mv.addObject("gc_id",gc_id);
     }
     if(null!=store_recommend&&!"".equals(store_recommend)){
       qo.addQuery("obj.store_recommend", new SysMap("store_recommend", CommUtil.null2Boolean(store_recommend)), "=");
       params="&&store_recommend="+CommUtil.null2Boolean(store_recommend);
       mv.addObject("store_recommend" ,store_recommend);
     }
     if(null!=bc_location&&!"".equals(bc_location)){
       qo.addQuery("obj.gc.bc_location",new SysMap("bc_location",CommUtil.null2String(bc_location)),"=");
       params="&&bc_location="+CommUtil.null2String(bc_location);
       mv.addObject("bc_location",bc_location);
     }else{
       qo.addQuery("obj.gc.bc_location",new SysMap("bc_location",CommUtil.null2String(1)),"=");
       params="&&bc_location=1";
       mv.addObject("bc_location",1);
     }
    ...
    /* params="&&brand_id="+CommUtil.null2Long(brand_id)+"&&gc_id="+CommUtil.null2Long(gc_id)+"&&store_recommend="+CommUtil.null2Boolean(store_recommend);*/
     ...
     map.put("PinOrSha",CommUtil.null2String(1));
     List gbs = this.goodsBrandService.query(
       "select obj from GoodsBrand obj where obj.deleteStatus=:deleteStatus and obj.PinOrSha=:PinOrSha order by obj.sequence asc", map, -1, -1);
     map.clear();
     map.put("deleteStatus",CommUtil.null2Boolean(false));
     if(null==bc_location||"".equals(bc_location)){
       map.put("bc_location",CommUtil.null2String(1));
     }else{
       map.put("bc_location",CommUtil.null2String(bc_location));
     }
     List gcs = this.goodsClassService.query(
       "select obj from GoodsClass obj where obj.parent.id is null and obj.deleteStatus=:deleteStatus and obj.bc_location =:bc_location order by obj.sequence asc", map, -1, -1);
