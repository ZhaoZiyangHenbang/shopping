 ...
 public class StoreManageAction
 {
   ...
   public ModelAndView store_list(HttpServletRequest request, HttpServletResponse response,String store_name,String store_status, String currentPage, String orderBy, String orderType)
   {
     ModelAndView mv = new JModelAndView("admin/blue/store_list.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
    ...
    /* String params = "";*/
     ...
     if(null!=store_name&&!"".equals(store_name)){
       ...
       param.append("&&store_name="+store_name);
     ...
     if(null!=store_status&&!"".equals(store_status)) {
       ...
       param.append("&&store_status=" + store_status);
     ...
     CommUtil.saveIPageList2ModelAndView(url + "/admin/store_list.htm", "", param.toString(), pList, mv);
     List grades = this.storeGradeService.query("select obj from StoreGrade obj order by obj.sequence asc", null, -1, -1);
   ...
   {
     ModelAndView mv = new JModelAndView("admin/blue/store_add.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
   ...
   {
     ModelAndView mv = new JModelAndView("admin/blue/store_new.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
     User user = this.userService.getObjByProperty("userName", userName);
     Store store = null;
     if (user != null)
       store = this.storeService.getObjByProperty("user.id", user.getId());
     if (user == null) {
       mv = new JModelAndView("admin/blue/tip.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
     ...
     throws Exception
   {
     ...
     store.setArea(area);
     store.setStore_lng(BigDecimal.valueOf(0.00));
     StoreClass sc = this.storeClassService
       .getObjById(Long.valueOf(Long.parseLong(sc_id)));
     store.setSc(sc);
     store.setTemplate("default");
    // if ((null !=grade_id) && (!"".equals(grade_id))) {
       store.setGrade(this.storeGradeService.getObjById(CommUtil.null2Long(1)));
    // }
     ...
     for (String id : ids) {
       if (!id.equals("")) {
         ...
         for (Goods goods : store.getGoods_list()) {
           ...
           goodCarts = this.goodsCartService.query("select obj from GoodsCart obj where obj.goods.id = :gid", map, -1, -1);
           Long ofid = null;
           Map map2;
           List goodCarts2;
           for (GoodsCart gc : goodCarts) {
             ...
             if (gc.getOf() != null) {
               ...
               if (goodCarts2.size() == 0) {
                 gc.getOf().setDeleteStatus(true);
                 this.orderFormService.update(gc.getOf());
               }
             }
           }
   ...
   public ModelAndView store_template(HttpServletRequest request, HttpServletResponse response) {
     ModelAndView mv = new JModelAndView("admin/blue/store_template.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
