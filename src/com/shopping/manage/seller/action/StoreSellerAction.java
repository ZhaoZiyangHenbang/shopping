 ...
 public class StoreSellerAction
 {
   ...
   public ModelAndView store_set(HttpServletRequest request, HttpServletResponse response) {
     ModelAndView mv = new JModelAndView(
       "user/second/seller_storeset.html",
       this.configService.getSysConfig(), 
   ...
   @SecurityMapping(display = false, rsequence = 0, title="店鋪廣告列表", value="/seller/store_theme.htm*", rtype="seller", rname="主题设置", rcode="store_theme_seller", rgroup="店铺设置")
   @RequestMapping({"/seller/store_adv.htm"})
   public ModelAndView store_adv(HttpServletRequest request, HttpServletResponse response, String ad_title, String currentPage, String orderBy, String orderType,String user_id) {
     ModelAndView mv = new JModelAndView("user/second/85adv_list.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
     ...
     if (!CommUtil.null2String(ad_title).equals("")) {
       ...
       bu.append("&ad_title="+ad_title);
     }
     if(user_id!=null&&!"".equals(user_id)){
       qo.addQuery("obj.ad_user.id", new SysMap("ad_user_id", CommUtil.null2Long(user_id)), "=");
       bu.append("&ad_user_id="+user_id);
     }
    /* qo.addQuery("obj.ad_status", new SysMap("ad_status", Integer.valueOf(0)), "=");*/
     ...
     CommUtil.saveIPageList2ModelAndView(url+"/seller/store_adv.htm", "", bu.toString(), pList, mv);
   ...
   @SecurityMapping(display = false, rsequence = 0, title="店鋪添加廣告", value="/seller/store_theme.htm*", rtype="seller", rname="主题设置", rcode="store_theme_seller", rgroup="店铺设置")
   @RequestMapping({"/seller/store_adv_add.htm"})
   public ModelAndView store_adv_add(HttpServletRequest request, HttpServletResponse response, String currentPage,String id) {
     ModelAndView mv = new JModelAndView("user/second/85adv_add.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
     if(null!=id&&!"".equals(id)){
       Advert adv=this.advertService.getObjById(CommUtil.null2Long(id));
       mv.addObject("adv",adv);
     }
     List aps = this.advertPositionService.query("select obj from AdvertPosition obj where obj.ap_shangjia=2", null, -1, -1);
     mv.addObject("aps", aps);
     mv.addObject("currentPage", currentPage);
     return mv;
   }

   @RequestMapping({"/seller/store_adv_del.htm"})
   public String store_adv_del(HttpServletRequest request, HttpServletResponse response, String currentPage,String id) {
     this.advertService.delete(CommUtil.null2Long(id));
     User user=SecurityUserHolder.getCurrentUser();
     return "redirect:/seller/store_adv.htm?currentPage="+currentPage+"&user_id="+user.getId();
   }
   ...
   public ModelAndView store_theme(HttpServletRequest request, HttpServletResponse response) {
     ModelAndView mv = new JModelAndView("user/second/seller_themesettings.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
