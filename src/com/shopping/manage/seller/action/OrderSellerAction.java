 ...
 public class OrderSellerAction
 {
   ...
   public ModelAndView order(HttpServletRequest request, HttpServletResponse response, String currentPage, String order_status, String order_id, String beginTime, String endTime, String buyer_userName)
   {
     ModelAndView mv = mv = new JModelAndView(
       "user/second/seller_ordelmanagement.html",
   ...
   public ModelAndView order_view(HttpServletRequest request, HttpServletResponse response, String id) {
     ...
     String gongsi="";
     String danhao="";
     ModelAndView mv = new JModelAndView("user/second/Buyer_Order_View.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
     String shopping_view_type = CommUtil.null2String( request.getSession().getAttribute( "shopping_view_type" ) );
     if ((shopping_view_type != null) && (!shopping_view_type.equals( "" )) && (shopping_view_type.equals( "wap" ))) {

       mv = new JModelAndView("../shop/newwap/logistics_details.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
     }
     ...
     try
     {
       URL url= new URL("http://api.kuaidi100.com/api?id="+userid+"&com="+gongsi+"&nu="+danhao+"&show=0&muti=1&order=asc");
     ...
     if (obj.getStore().getId()
       ...
       for (GoodsCart gc : obj.getGcs()) {
         ...
         if (inventory_type.equals("all")) {
           ...
           List list = Json.fromJson(ArrayList.class,
   ...
   public ModelAndView order_shipping(HttpServletRequest request, HttpServletResponse response, String id, String currentPage) {
     ...
     if (obj.getStore().getId().equals(SecurityUserHolder.getCurrentUser().getStore().getId())) {
       ...
       List expressCompanys = this.expressCompayService.query("select obj from ExpressCompany obj where obj.company_status=:status order by company_sequence asc",
         params, -1, -1);
   ...
   public String order_refund_save(HttpServletRequest request, HttpServletResponse response, String id, String currentPage, String refund, String refund_log, String refund_type) {
     ...
     if (obj.getStore().getId().equals(SecurityUserHolder.getCurrentUser().getStore().getId())) {
       ...
       obj.setOrder_status(22);
       this.orderFormService.update(obj);
 
       String type = "预存款";
       if (type.equals(refund_type)) {
         User seller = this.userService.getObjById(obj.getStore().getUser().getId());
         seller.setAvailableBalance(BigDecimal.valueOf(CommUtil.subtract(seller.getAvailableBalance(), BigDecimal.valueOf(CommUtil.null2Double(refund)))));
         this.userService.update(seller);
         User buyer = obj.getUser();
         buyer.setAvailableBalance(BigDecimal.valueOf(CommUtil.add(buyer.getAvailableBalance(), BigDecimal.valueOf(CommUtil.null2Double(refund)))));
   ...
   public ModelAndView order_evaluate(HttpServletRequest request, HttpServletResponse response, String id, String currentPage) {
     ...
     if (obj.getStore().getId()
       .equals(SecurityUserHolder.getCurrentUser().getStore().getId())) {
       List<Evaluate> evaluateList=obj.getEvas();
       List<Evaluate> evas=new ArrayList<>();
       if (evaluateList!=null&&evaluateList.size()>0) {
         for (int i = 0; i <evaluateList.size() ; i++) {
           if (evaluateList.get(i).getEvaluate_type().equals("goods")) {
              evas.add(evaluateList.get(i));
           }
         }
       }
       mv.addObject("obj", obj);
   ...
   public ModelAndView order_evaluate_save(HttpServletRequest request, HttpServletResponse response, String id, String evaluate_info, String evaluate_seller_val) {
     ...
     mv.addObject("op_title", "对买家评价回复成功！");
     mv.addObject("url", CommUtil.getURL(request) + "/seller/order.htm");
     return mv;
   }
