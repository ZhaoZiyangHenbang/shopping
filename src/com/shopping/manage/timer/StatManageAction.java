...
public class StatManageAction {
	...
	public void execute() throws Exception {
		...
		List<OrderForm> weifukuan = this.orderFormService.query("select obj from OrderForm obj where obj.addTime<=:addTime and obj.order_status < 15", params, -1, -1);
		for(OrderForm of:weifukuan){
			//订单状态改为0取消订单
			...
			ofl.setState_info("24小时未付款自动取消");
			this.orderFormLogService.save(ofl);
			//根据订单id遍历购物车查找把购物车中商品状态改变为
			params.clear();
			params.put("of_id",of.getId());
			List<GoodsCart> list=this.goodsCartService.query("select obj from GoodsCart obj where obj.of.id=:of_id",params,-1,-1);
			for(GoodsCart gc:list){
				gc.setGc_status(0);
				gc.setEvaluate_status("0");
				gc.setOf_type("0");
				this.goodsCartService.update(gc);
				//根据商品id改变商品的库存
				Long id=gc.getGoods().getId();
				if(null!=id) {
					Goods good = this.goodsService.getObjById(id);
					if(null!=good) {
						good.setGoods_inventory(good.getGoods_inventory() + gc.getCount());
		...
		params.put("shipTime", cal.getTime());
		List<OrderForm> confirm_ofs = this.orderFormService.query(//查询运输时间小于设定的时间的订单集合
		...
		for (OrderForm of : confirm_ofs) {
			...
			/*	-------------收货完计算用户的等级--------------*/
				User u = of.getUser();
				//当前订单的用户的总金额
				BigDecimal one = u.getPay_money().add(of.getGoods_amount());
				of.getUser().setPay_money(one);
				//当前用户的下线数
				Integer num = 0;
				params.clear();
				params.put("parent", u);
				...
				if(Integer.valueOf(creditMoney)>Integer.valueOf(creditPerson)){
					u.setUser_credit(CommUtil.null2Int(creditMoney));
				}else{
					u.setUser_credit(CommUtil.null2Int(creditPerson));
				}
